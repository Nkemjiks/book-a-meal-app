...
const mealOrderController = {
  ...
  async placeOrder(req, res) {
    ...
    menu.getMeals({ where: { id: mealIds } }).then((mealsPresent) => {
      if (mealsPresent.length !== mealIds.length) {
    ...
    return models.order
      ...
      .then((order) => {
        setTimeout(() => {
          order.getMeals().then((adds) => {
            const filterOrder = filterPlaceOrder(order, adds);
